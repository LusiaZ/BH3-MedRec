...
from Hypergraph_model import BH3, load_icd_ndc_relationships
from util import llprint, multi_label_metric, ddi_rate_score, get_n_params

torch.manual_seed(1203)

model_name = "BH3"
resume_name = "saved/BH3/Epoch_49_JA_0.5073_DDI_0.0858.model"
...
prescriptions_df = pd.read_csv('../data/GU_input/gu_prescriptions_cleaned.csv')

unique_ndc_count = prescriptions_df['ndc'].nunique()

parser.add_argument("--num_drugs", type=int, default=unique_ndc_count, help="number of drugs")
parser.add_argument("--drug_embedding_dim", type=int, default=64, help="dimension of drug embeddings")
...
def eval(model, data_eval, voc_size, epoch):
    ...
    # ddi rate
    ddi_rate = ddi_rate_score(smm_record, path="../data/GU_output/ddi_A_final.pkl")
    ...
    llprint(
        "\nDDI Rate: {:.4f}, Jaccard: {:.4f},  PRAUC: {:.4f}, AVG_PRC: {:.4f}, AVG_RECALL: {:.4f}, AVG_F1: {:.4f}, AVG_MED: {:.4f}\n".format(
            ...
            float(np.mean(avg_f1)), float(med_cnt) / float(visit_cnt)
        )
    )
...
# hypergraph adj matrix construction
...
def main():
    ...
    for epoch in range(EPOCH):
        ...
        print(
            "Epoch: {}, DDI: {:.4f}, Med: {:.4f}, Ja: {:.4f}, F1: {:.4f}, PRAUC: {:.4f}".format(
                epoch + 1,
